...
export default function ListingForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/farmers/listings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ product, quantityBags: quantity, pricePerBag: price }),
    });
    const data = await res.json();
    if (!res.ok) setError(data.error || 'Failed to create listing');
    else { setSuccess('Listing created successfully!'); setQuantity(''); setPrice(''); }
    setLoading(false);
  };
