    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={`w-72 bg-[#00703C] flex flex-col fixed top-0 left-0 bottom-0 shadow-xl z-50 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          <Link href="/buyer/dashboard" onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm ${pathname === '/buyer/dashboard' ? 'bg-white/15 text-white font-semibold' : 'text-green-100 hover:bg-white/10'}`}>
