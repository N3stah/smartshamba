  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-gray-200 pb-4 gap-4">
        ...
        <button onClick={() => { setShowForm(!showForm); setEditingProvider(null); setFormData(emptyForm); }} className="bg-[#00703C] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#00582f] flex items-center gap-2">
      ...
      {tempPassword && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-green-800 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm mt-1">Temporary Password: <span className="font-mono bg-green-100 px-2 py-0.5 rounded">{tempPassword}</span></p>
      ...
      {(showForm || editingProvider) && (
        <form onSubmit={editingProvider ? handleEditProvider : handleCreateProvider} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
          ...
          <div className="flex gap-2 mt-4">
            <button type="submit" className="bg-[#00703C] text-white px-4 py-2 rounded-lg text-sm font-medium">{editingProvider ? 'Save Changes' : 'Create Provider'}</button>
          ...
          <div key={p.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <div className="flex flex-col md:flex-row justify-between mb-4">
              ...
              <div className="flex flex-col items-start md:items-end gap-2 mt-2 md:mt-0">
                <div className="flex gap-2">
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${p.verificationStatus === 'VERIFIED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                ...
                <div className="flex gap-2">
                  ...
                  {p.verificationStatus !== 'VERIFIED' && (
                    <button onClick={() => handleAction(p.id, 'verify')} className="text-xs bg-green-50 text-green-700 px-2 py-1 rounded border border-green-200 flex items-center gap-1">
            ...
            <div className="border-t border-gray-100 pt-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <button onClick={() => setVehicleForms({ ...vehicleForms, [p.id]: !vehicleForms[p.id] })} className="text-xs text-[#00703C] flex items-center gap-1">
              ...
              {vehicleForms[p.id] && (
                <form onSubmit={(e) => handleAddVehicle(e, p.id)} className="bg-gray-50 p-4 rounded-lg mb-4 grid grid-cols-1 md:grid-cols-3 gap-2">
                  ...
                  <div className="col-span-full flex gap-2">
                    <button type="submit" className="bg-[#00703C] text-white px-3 py-1 rounded text-xs">Save Vehicle</button>
                    ...
                    {editingVehicle?.vehicle.id === v.id ? (
                      <form onSubmit={handleEditVehicle} className="grid grid-cols-1 md:grid-cols-4 gap-2 w-full items-center">
                        ...
                        <div className="flex gap-1">
                          <button type="submit" className="bg-[#00703C] text-white px-2 py-1 rounded text-xs">Save</button>
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={async () => {
                            ...
                            fetchProviders();
                          }} className={`text-xs px-2 py-0.5 rounded ${v.isActive ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'}`}>
